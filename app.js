  ...
  items.forEach(item =>
    aiAnalyze(item)
      ...
      .forEach(x =>
        insights.push({
          ...x,
          cultivation: item.c.name,
          cultivationId: item.c.id
        })
      )
  );
    ...
    .map(i => `
      <button class="ai-insight ${i.type}" data-cultivation-id="${escapeHtml(i.cultivationId)}">
        ...
        <span class="ai-body">
          ...
          <small>${escapeHtml(i.cultivation)} · ${escapeHtml(i.evidence)}</small>
          <em>${escapeHtml(i.text)}</em>
        </span>
        <b>›</b>
      </button>
    `)
    .join("")
    || `<div class="good-state">✨ <strong>Tudo tranquilo.</strong><span>Nenhum alerta importante no momento.</span></div>`;
}
...
const temperature = Number.isFinite(Number(val("temperature")))
  ...
  $("#modalContent").innerHTML=`<p class="eyebrow">DIÁRIO · DIA ${day}</p><h2>Registrar observação</h2>
    <form id="logForm" class="form-grid">
      ...
      <label>Peso (${weightUnit})<input id="logWeight" type="number" min="0" step="${weightUnit === "oz" ? "0.01" : "0.1"}" value="${escapeHtml(weight)}" placeholder="${weightUnit} (opcional)"></label>
